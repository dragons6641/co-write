...
app.use('/admin', express.static(__dirname+'/views_si'));
...
app.post('/si/report', function(req, res){
  ...
  var user_id = req.session.user_id;
  var sql = "select partner_id from letter where user_id=?";
  var params = [user_id];
  conn.query(sql, params, function(err, rows, fields){
...
//report_finish
app.get('/si/report_finish', function(req, res){
  res.render('si_report_finish', {});
});
